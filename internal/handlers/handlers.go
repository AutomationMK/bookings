...
func (m *Repository) ReservationSummary(w http.ResponseWriter, r *http.Request) {
	...
	if !ok {
		m.App.ErrorLog.Println("cannot get item from session in ReservationSummary")
...
func (m *Repository) AvailabilityJSON(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		resp := jsonResponse{
			OK:      false,
			Message: "Internal sever error parsing form",
		}
	...
	if err != nil {
		resp := jsonResponse{
			OK:      false,
			Message: "Internal sever error parsing arrival_date",
		}
	...
	if err != nil {
		resp := jsonResponse{
			OK:      false,
			Message: "Internal sever error parsing departure_date",
		}
	...
	if err != nil {
		resp := jsonResponse{
			OK:      false,
			Message: "Internal sever error parsing room_id",
		}
	...
	if err != nil {
		resp := jsonResponse{
			OK:      false,
			Message: "Internal sever error searching for room availability",
		}
...
func (m *Repository) BookRoom(w http.ResponseWriter, r *http.Request) {
	...
	arrivalDate, err := time.Parse(layout, ad)
	if err != nil {
		m.App.Session.Put(r.Context(), "error", "Can't parse arrival date from get query!")
		m.App.ErrorLog.Println("Can't parse arrival date from get query!")
		http.Redirect(w, r, "/", http.StatusTemporaryRedirect)
		return
	}
	departureDate, err := time.Parse(layout, dd)
	if err != nil {
		m.App.Session.Put(r.Context(), "error", "Can't parse departure date from get query!")
		m.App.ErrorLog.Println("Can't parse departure date from get query!")
		http.Redirect(w, r, "/", http.StatusTemporaryRedirect)
		return
	}
...
func (m *Repository) ChooseRoom(w http.ResponseWriter, r *http.Request) {
	...
	if !ok {
		...
		m.App.ErrorLog.Println("Unable to get reservation from session!")
		http.Redirect(w, r, "/", http.StatusTemporaryRedirect)
		return
	}
...
// Logout logs a user out and resets session
